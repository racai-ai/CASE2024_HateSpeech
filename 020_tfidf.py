...
dataTrain = pd.read_csv(args.train, sep='\t', header=0, keep_default_na=False, na_values=['_________________'])
dataTrain = dataTrain.loc[:,['text','label']]
labels=[str(l) for l in dataTrain.label.unique()]

dataTest = pd.read_csv(args.test, sep='\t', header=0, keep_default_na=False, na_values=['_________________'])
